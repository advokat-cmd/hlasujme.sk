...
export default function CreatePollClient() {
  ...
  React.useEffect(() => {
    ...
    fetch("/api/admin/building")
      ...
      .then((data) => {
        if (data.success) {
          setUnits((data.units || []).filter((unit: { status: string }) => unit.status === "active"));
          if (data.building) {
  ...
  const noEmailUnits = units.filter(u => !u.email?.trim() && (u.coMode !== "internal"
    || !u.owners?.some((owner: { email?: string }) => owner.email?.trim())));
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (failedUploads.length > 0) {
          alert(
            `Návrh hlasovania bol uložený, ale niektoré prílohy sa nepodarilo nahrať:\n\n${failedUploads.join("\n")}\n\nPozvánky ešte neboli odoslané. Doplňte prílohy v záložke „Podklady a dokumenty“ a potom návrh spustite.`
          );
          router.push(`/admin/poll/${pollId}?tab=documents`);
          router.refresh();
          return;
        }

        const activation = await fetch(`/api/admin/poll/${pollId}/activate`, { method: "POST" });
        const activationData = await activation.json();
        if (!activation.ok) {
          alert(`Návrh bol uložený, ale spustenie zlyhalo: ${activationData.error || "Skúste návrh spustiť z jeho detailu."}`);
        } else if (activationData.failedCount > 0) {
          alert(`Hlasovanie bolo spustené. ${activationData.failedCount} pozvánok sa nepodarilo odoslať. Skontrolujte pozvánky v detaile hlasovania.`);
        }
      ...
      if (createdPollId) {
        alert("Návrh bol uložený, ale dokončenie sa nepodarilo overiť. Skontrolujte jeho stav v detaile hlasovania.");
        router.push(`/admin/poll/${createdPollId}?tab=documents`);
        router.refresh();
      } else setError("Chyba sieťového pripojenia. Pred opakovaním skontrolujte zoznam návrhov, či sa hlasovanie už nevytvorilo.");
      setLoading(false);
    }
  };
  ...
  const validateStep = () => {
    if (step === 0) {
      ...
      const start = new Date(basics.start).getTime();
      const end = new Date(basics.end).getTime();
      if (!Number.isFinite(start) || !Number.isFinite(end) || end <= start || end <= Date.now()) {
        alert("Zadajte platný začiatok a koniec hlasovania. Koniec musí byť po začiatku a v budúcnosti.");
        return;
      }
    }
    if (step === 1) {
        ...
        {step === 0 && (
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 16 }}>
              <FormRow label="Začiatok hlasovania" hint="Predvolene aktuálny čas. Dátum a čas sa zadáva podľa časového pásma vášho zariadenia.">
                <input
                  type="datetime-local"
                  style={fieldStyle}
                  value={basics.start}
          ...
          <div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {[
                ...
                { title: "Určený zástupca / právnická osoba", desc: "Jeden odkaz na e-mail jednotky", count: fmtUnits(repLegalCount) },
          ...
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1.2fr 1fr", gap: 22 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {unitsCount === 0 ? (
                    ...
                    <>
                      ...
                      <div style={{ display: "flex", gap: 10, alignItems: "flex-start", fontSize: "13.5px" }}>
                        <Ic name="checkCircle" size={18} style={{ color: "var(--agree)", flexShrink: 0, marginTop: 1 }} />
                        <span style={{ color: "var(--ink)" }}>Zahrnuté sú aktívne jednotky aktuálneho registra</span>
                      </div>
                      <div style={{ display: "flex", gap: 10, alignItems: "flex-start", fontSize: "13.5px" }}>
                        <Ic name="checkCircle" size={18} style={{ color: "var(--agree)", flexShrink: 0, marginTop: 1 }} />
                        <span style={{ color: "var(--ink)" }}>Použijú sa režimy spoluvlastníctva nastavené v registri</span>
                      </div>
              ...
              <div style={{ background: "var(--paper-2)", borderRadius: 12, padding: 18 }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", gap: 12, padding: "7px 0", borderTop: "1px solid var(--line)", fontSize: 13 }}>
                  <span style={{ color: "var(--ink-soft)" }}>Pozvánky</span>
                  <span style={{ fontWeight: 600, textAlign: "right" }}>E-mailom po nahratí príloh</span>
                </div>
              </div>
            </div>
          </div>
